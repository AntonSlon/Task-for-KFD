...
object Terminal{
    ...
    private fun coefficientsChanger(pairs: Int){
        ...
        coefficientsMap[changerMap[pairs]!!] = (mapSign[Random.nextInt(0, 1)].toString() + coefficient.toString()).toFloat()
        println((mapSign[Random.nextInt(0, 2)].toString() + coefficient.toString()).toFloat())
    }

    private fun checkCurrency(pairs: Int, amountOfMoney: Float, currency: String, anotherCurrency: String){
        val value = changerMap[pairs]!!
        if (MyWallet.money[currency]!! - amountOfMoney < 0){throw ValueException("Недостаточно средств на балансе")}
        if (currencyMap[anotherCurrency]!! - amountOfMoney / coefficientsMap[value]!! < 0){throw ValueException("Недостаточно средств на счете банка")}
    }
    ...
    fun operation(pairs: Int, amountOfMoney: Float, currency: String) {
        ...
        when (currency){
            currency1 -> {
                checkCurrency(pairs, amountOfMoney, currency2, currency1)
            ...
            currency2 -> {
                checkCurrency(pairs, amountOfMoney, currency1, currency2)
